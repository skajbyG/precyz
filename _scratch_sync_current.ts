...
function extractOverUnder(bookmakers: any[]): { line: number; over: number; under: number; sources: number; note: string } | null {
  ...
  return {
    line: bestPoint, over: +(overSum / sum).toFixed(4), under: +(underSum / sum).toFixed(4), sources: count,
    note: `Mezinárodní odhad z ${count} zahraničních sázkovek (Pinnacle, Bet365 a podobné) — appka na tenhle trh nemá data přímo z Tipsport/Fortuna/Betano.`,
  };
}
